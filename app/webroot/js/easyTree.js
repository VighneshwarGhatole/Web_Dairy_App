...
(function ($) {
    $.fn.EasyTree = function (options) {
        ...
        var createInput = $('<div class="input-group"><input type="text" maxlength="100" class="form-control"><span class="input-group-btn"><button type="button" class="btn btn-default btn-success confirm"></button> </span><span class="input-group-btn"><button type="button" class="btn btn-default cancel"></button> </span> </div> ');
        ...
        this.each(function () {
            ...
            // addable
            if (options.addable) {
                $(easyTree).find('.easy-tree-toolbar').append('<h4>Use Tools for Add, Edit and Delete.</h4><div class="create"><button class="btn btn-default btnprp btn-success addnode"><span class="glyphicon glyphicon-plus"></span></button></div> ');
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {                        
                        ...
                        if(selected.length === 0)
							...
							var pid = selected[0]['id'];
							
                        var addModuleItems = addMItems(pid,$(createInput).find('input').val());                        
                       
						//console.log(addModuleItems);
                        var json = JSON.parse(addModuleItems);
						
                        if (json.status == "error") {
							$(easyTree).prepend('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong></strong><span class="alert-content">'+json.message+'</span> </div>');
							return;
						}

                        var item = $('<li id="' + json.id + '"><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
            ...
            // editable
            if (options.editable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="edit"><button class="btn btn-default btnprp btn-primary disabled"><span class="glyphicon glyphicon-edit"></span></button></div> ');
                        ...
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {								
                                ...
                                if ($(editor).val() !== '') {
									...
									var editModuleItems = editMItems(pid,$(editor).val());                        
								   
									//console.log(addModuleItems);
									var json = JSON.parse(editModuleItems);
            ...
            // deletable
            if (options.deletable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="remove"><button class="btn btn-default btnprp btn-danger disabled"><span class="glyphicon glyphicon-remove"></span></button></div> ');
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {  
								...
								var deleteModuleItems = deleteMItems(id);                        
							   
								//console.log(addModuleItems);
								var json = JSON.parse(deleteModuleItems);
            ...
            // Insert into db
            var addMItems = function(pid,item){				
				var rdata = '';
				var values = {batch_id: batchId,parent_id: pid,name: item};
				$.ajax({
					url: webURL + "Modules/add",
					type: "post",
					data: values ,
					async: false,
				...
				//return {"status":"error","message":"Sorry!! Not Authorized.."};
				return rdata;
			};
			 var editMItems = function(id,item){				
				var rdata = '';
				var values = {id: id,name: item};
				$.ajax({
					url: webURL + "Modules/edit",
					type: "post",
					data: values ,
					async: false,
					success: function (response) {
			 ...
			 var deleteMItems = function(id){				
				var rdata = '';
				var values = {id: id};
				$.ajax({
					url: webURL + "Modules/delete",
					type: "post",
					data: values ,
					async: false,
					success: function (response) {
   ...
   /* function addModule(pid='',item=''){
			//return {"status":"error","errors":"Sorry!! Not Authorized"};
